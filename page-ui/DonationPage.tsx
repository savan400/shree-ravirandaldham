...
// ── Donation Card ─────────────────────────────────────────────────────────────
const DonationCard = ({ cat }: { cat: DonationCategory }) => {
    ...
    const handlePay = () => {
        ...
        // hook up payment gateway here
        alert(`Proceeding to pay ₹${amt} for ${cat.title}`);
    };
    ...
    return (
        <div className={don.donCard}>
            ...
            <div className={don.donAmounts}>
                ...
                {cat.hasCustom && (
                    <label className={`${don.donOption} ${selected === 'custom' ? don.donOptionActive : ''}`}>
                        <input
                            type="radio"
                            name={cat.id}
                            value="custom"
            ...
            {!selected && cat.hasCustom && (
                <div className={don.donHint}>Minimum amt. {cat.minCustom} or higher</div>
            )}
            ...
            <div className={don.container}>
                <div className={visibleClass("header", visible)}>
                    ...
                    <div className={don.donGrid}>
                        {categories.map((cat) => (
                            <DonationCard key={cat.id} cat={cat} />
                        ))}
                    </div>
