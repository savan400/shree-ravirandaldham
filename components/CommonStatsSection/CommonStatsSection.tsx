...
import styles from "./CommonStatsSection.module.css";
import CommonTitle from "../CommonTitle/CommonTitle";
import PageBackgroundDecorations from "../PageBackgroundDecorations/PageBackgroundDecorations";

// ─── Types ────────────────────────────────────────────────────────────────────

export type IconKey = "height" | "floors" | "rooms" | "sqft" | "people";
...
export interface StatsSectionProps {
    ...
    footerNote?: string;
    stats: StatItem[];
    countDuration?: number; // ms, default 2200
...
function formatCount(count: number, stat: StatItem): string {
    const atTarget = count >= stat.value;
    const suffix = stat.suffix ?? "";
    if (stat.value >= 10_000) {
        const k = (count / 1000).toFixed(atTarget ? 0 : 1);
        return `${k}K${atTarget ? suffix : ""}`;
    }
    return `${count.toLocaleString()}${atTarget ? suffix : ""}`;
}

// ─── Stat Card ────────────────────────────────────────────────────────────────

const StatCard: React.FC<StatCardProps> = ({ stat, inView, index, countDuration }) => {
    const count = useCountUp(stat.value, countDuration, inView);
...
const CommonStatsSection: React.FC<StatsSectionProps> = ({
    title,
    subtitle,
    footerNote,
    stats,
