...
export default function CMSPageForm({
  ...
  const [images, setImages] = useState<GridImage[]>(() => [
    ...(page?.images || []).map((img: any, i) => {
      const displayUrl = img.url; // This is the signed URL from resolveCMSPage
      const rawKey = img.key; // This is the raw key we need for DB
      return {
        id: `existing-${i}-${rawKey.slice(-12)}`,
        isNew: false,
        url: displayUrl,
        key: rawKey,
        caption: img.caption,
        description: img.caption, // For ImageUploadGrid compatibility
  ...
  const handleDescriptionChange = useCallback(
    (id: string, lang: "en" | "hi" | "gu", value: string) => {
      setImages((prev) =>
        prev.map((img) =>
          img.id === id
            ? {
  ...
  const updateAnalyticsTranslation = (
    index: number,
    ...
    setFormData((prev) => {
      const newAnalytics = [...prev.analytics];
      const currentField = (newAnalytics[index] as any)[field] || {
        en: "",
        hi: "",
        gu: "",
      };
  ...
  const validateForm = () => {
    ...
    if (!formData.key.trim()) {
      ...
      newErrors.push(
        "Page Key must be lowercase alphanumeric and hyphens only",
      );
    }
    ...
    if (!formData.badgeText.en.trim())
      newErrors.push("English Badge Text is required");
    if (!formData.badgeText.hi.trim())
      newErrors.push("Hindi Badge Text is required");
    if (!formData.badgeText.gu.trim())
      newErrors.push("Gujarati Badge Text is required");

    if (!formData.description.en.trim())
      newErrors.push("English Description is required");
    if (!formData.description.hi.trim())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // map images back to the expected structure
      const currentImages = images
        .filter((img) => !img.isNew)
        .map((img: any) => ({
          url: img.key || img.url, // Send raw key back to DB
          caption: img.caption,
        }));
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Define the structure and content for this page key.
          </p>
        </div>
        <button
          onClick={onClose}
          ...
          <div className="space-y-6">
            <Card className="p-6 space-y-4">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-bold text-gray-700">
                    Badge Text ({activeTab.toUpperCase()}) *
                  </label>
                  <input
                    type="text"
            ...
            <Card className="p-6">
              ...
              <div className="space-y-4">
                {formData.analytics.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-4 border border-orange-100 rounded-xl bg-orange-50/10 space-y-3 relative group"
                  >
                    <button
                      type="button"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <input
                        ...
                        value={item.key}
                        onChange={(e) =>
                          updateAnalyticsField(idx, "key", e.target.value)
                        }
                      ...
                      <input
                        ...
                        value={item.value}
                        onChange={(e) =>
                          updateAnalyticsField(idx, "value", e.target.value)
                        }
                        className="bg-white border text-xs px-3 py-2 rounded-lg"
                      />
                    </div>
