...
export default getRequestConfig(async ({ requestLocale }) => {
  ...
  try {
    ...
    console.log(
      `[i18n] Fetching remote translations from: ${backendUrl}/translations?locale=${locale}`,
    );

    const res = await axios.get(`${backendUrl}/translations?locale=${locale}`);

    if (res.status === 200) {
      messages = res.data;
      console.log(
        `[i18n request] Loaded ${Object.keys(messages).length} remote sections for locale "${locale}"`,
      );
    } else {
      console.warn(
        `[i18n] Backend returned status ${res.status} for locale "${locale}"`,
      );
    }
  } catch (err) {
    ...
    if (err instanceof Error && (err as any).code === "ECONNREFUSED") {
      console.warn(
        `[i18n] Backend connection refused at ${process.env.NEXT_PUBLIC_API_URL}. Proceeding with empty messages.`,
      );
    } else {
